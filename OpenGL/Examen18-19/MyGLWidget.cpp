...
//  glViewport (0, 0, ample, alt);
  ...
  // Activem el VAO per a pintar el cub -> Paredes
...
void MyGLWidget::modelTransformModel ()
{
  ...
  TG = glm::rotate(TG, -float((M_PI/180)*90), glm::vec3(0,1,0));
...
{
  glm::mat4 TG(1.f);  // Matriu de transformació
  TG = glm::translate (TG, glm::vec3(-9.9,0.0, 0.0));
  TG = glm::scale(TG, glm::vec3(0.2, 2.0, 14));
  glUniformMatrix4fv (transLoc, 1, GL_FALSE, &TG[0][0]);
}
...
void MyGLWidget::projectTransform ()
{
  ...
  if (!primerapersona)Proj = glm::perspective(FOV, ra, radiEsc, 3.0f*radiEsc);
  else
  {
      Proj = glm::perspective(FOV, ra, 0.1f, -25.f);
  }
  //::cout << "Ra: " << ra << std::endl;
...
{
  glm::vec3 OBS(posNinot.x-0.01, posNinot.y+4, posNinot.z);
  glm::vec3 VRP(posNinot.x-5, 1, posNinot.z);
  glm::vec3 UP(0,1,0);
...
void MyGLWidget::keyPressEvent(QKeyEvent* event) 
{
  ...
  switch (event->key()) {
    case Qt::Key_Left: { // moviment del ninot
      if (posNinot.z < 5) posNinot.z += 0.5;
      viewTransform();
      break;
    }
  ...
  case Qt::Key_Up: { // moviment del ninot
      if (posNinot.x > -9) posNinot.x -= 0.5;
      viewTransform();
    break;
  }
...
void MyGLWidget::mousePressEvent (QMouseEvent *e)
{
  ...
  if (e->button() & Qt::LeftButton && !primerapersona &&
...
void MyGLWidget::creaBuffersModel(Model &model, const char *fileName, GLuint *VAO,  float &escala, glm::vec3 &centreBase)
{
  ...
  //if (legoman) escalaModel = escala*4;
  if (legoman) escala = escalarModel(model, 4.0);
...
float MyGLWidget::escalarModel(const Model &m, float f)
{
    float miny, maxy;
...
void MyGLWidget::calculaRadiEsc()
{
    ...
    Pmax.x = 10, Pmax.y = 3, Pmax.z = 7;
    Pmin.x = -10, Pmin.y = -1, Pmin.z = -7;
    // Pared1 = Pmax.x = 10, Pmin.x = -10 / Pmax.y = 1, Pmin.y = -1 / Pmax.z = -6.8, Pmin.z = -7.0
    // Pared2 = Pmax.x = 10, Pmin.x = -10 / Pmax.y = 1, Pmin.y = -1 / Pmax.z = 7.0, Pmin.z = 6.8
    // Pared3 = Pmax.x = -9.8, Pmin.x = -10 / Pmax.y = 1, Pmin.y = -1 / Pmax.z = 7.0, Pmin.z = -7
    // Legoman = Pmax.y = 4, Pmin.y = 0
