...
void MyGLWidget::keyPressEvent(QKeyEvent* event)
{
  ...
  case Qt::Key_O: {
      decideix=!decideix;   // Positiu (p.def) -> Perspectiva, Negatiu -> Ortogonal
      projectTransform();
      break;
  }
...
void MyGLWidget::mousePressEvent(QMouseEvent *e)
{
    ...
    // Sólo se mueve con el botón izquierdo y sin usar modificadores
    if (e->buttons() == Qt::LeftButton && !(e->modifiers() & (Qt::ShiftModifier | Qt::AltModifier | Qt::ControlModifier))) CurrentAction = ROTATE;
}
...
void MyGLWidget::mouseMoveEvent(QMouseEvent *e)
{
    if (CurrentAction == ROTATE){
        ...
        float x = angleX - antigaX;
        float y = angleY - antigaY;
