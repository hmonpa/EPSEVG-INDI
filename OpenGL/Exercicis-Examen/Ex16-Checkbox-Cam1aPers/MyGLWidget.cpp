...
void MyGLWidget::iniCamera ()
{
  ...
  posFocus = glm::vec3(posPatriX,0.5,0);      // posición Patricio1
...
void MyGLWidget::modelTransformPatricio ()
{
  ...
  TG = glm::translate(TG, glm::vec3(posPatriX,-0.5,0));
  TG = glm::scale(TG, glm::vec3(escala, escala, escala));
  TG = glm::rotate(TG, float(M_PI)/2, glm::vec3(0,1,0));        // rotación X+
...
{
    glm::mat4 TG(1.f);  // Matriu de transformació
    TG = glm::translate(TG, glm::vec3(2, 0.25, 1));
...
void MyGLWidget::projectTransform ()
{
  ...
  if (!primeraPersona)
    ...
    Proj = glm::perspective(FOV, ra, 0.1f, 10.f);
...
void MyGLWidget::viewTransform ()
{
  ...
  glm::vec3 OBS(posPatriX, 0.5, 0);                             // cámara posicionada encima del Patricio1
  glm::vec3 VRP(2, 0.25, 1);                                    // VRP = Centro Patricio 2
  glm::vec3 UP(0,1,0);                                          // Vector UP = Eje Y
...
void MyGLWidget::keyPressEvent(QKeyEvent* event) 
{
  ...
  switch (event->key()) {
    ...
    case Qt::Key_A: { // Patricio1 se mueve X+
      if (posPatriX < 2) posPatriX += 0.1;
      posFocus.x = posPatriX;
      glUniform3fv(posFocusLoc, 1, &posFocus[0]);
      viewTransform();
      break;
    }
    case Qt::Key_D: { // Patricio1 se mueve X-
        if (posPatriX > -2) posPatriX -= 0.1;
        posFocus.x = posPatriX;
        glUniform3fv(posFocusLoc, 1, &posFocus[0]);
        viewTransform();
    ...
    case Qt::Key_C: { // Patricio1 se mueve X-
...
void MyGLWidget::primera_Persona()
{
    makeCurrent();
    primeraPersona = !primeraPersona;
    viewTransform();
...
void MyGLWidget::createBuffersPatricio ()
{
  ...
  calculaCapsaModel ();
  escala = escalarModel(patr, 1);
  escala2 = escalarModel(patr, 2.5);
