...
void MyGLWidget::viewTransform ()
{
  ...
  {
      glm::mat4 View;
      glm::vec3 OBS = glm::vec3(0, 18, 0);
      glm::vec3 VRP = glm::vec3(centreEsc);
...
void MyGLWidget::keyPressEvent(QKeyEvent* event) 
{
  ...
  switch (event->key()) {
    case Qt::Key_Left: { // moviment del ninot
      if (posPilota.z < 6) posPilota.z += 0.5;
      comprovaLego();
      //std::cout << "Pos pilota: " << posPilota.x << "," << posPilota.y << "," << posPilota.z << std::endl;
      break;
    }
      ...
      case Qt::Key_Down: { // moviment del ninot
        if (posPilota.x < 9) posPilota.x += 0.5;
        comprovaLego();
        //std::cout << "Pos pilota: " << posPilota.x << "," << posPilota.y << "," << posPilota.z << std::endl;
        break;
      }
...
void MyGLWidget::comprovaLego()
{
    if (posPilota.x < -6 and posPilota.x > -7.5)
    {
        if (posPilota.z > 2 and posPilota.z < 4)
        {
            legoAss1 = true;
            creaBuffersLego();
        }
...
{
  modelLego = true;
  if(!legoAss1) creaBuffersModel(lego, "./models/legoman.obj", &VAO_Lego, escalaModel, centreBaseModel);
  else if (legoAss1) creaBuffersModel(lego, "./models/legoman-assegut.obj", &VAO_Lego, escalaModel, centreBaseModel);

  if(!legoAss2) creaBuffersModel(lego, "./models/legoman.obj", &VAO_Lego2, escalaModel, centreBaseModel);
  else if (legoAss2) creaBuffersModel(lego, "./models/legoman-assegut.obj", &VAO_Lego2, escalaModel, centreBaseModel);
}
